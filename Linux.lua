...
function Linux.Create(config)
    ...
    ConfirmationDialog.BackgroundTransparency = 0.5
    ...
    local cancelButtonClicked = false
    CancelButton.MouseButton1Click:Connect(function()
        if not cancelButtonClicked then
            cancelButtonClicked = true
            TweenService:Create(CancelButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(0, 100, 220)}):Play()
            task.wait(0.3)
            TweenService:Create(CancelButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(15, 15, 15)}):Play()
            cancelButtonClicked = false
        end
        HideConfirmationDialog()
    end)
    
    local confirmButtonClicked = false
    ConfirmButton.MouseButton1Click:Connect(function()
        if not confirmButtonClicked then
            confirmButtonClicked = true
            TweenService:Create(ConfirmButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(0, 100, 220)}):Play()
            task.wait(0.3)
            TweenService:Create(ConfirmButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(15, 15, 15)}):Play()
    ...
    function LinuxLib.Tab(config)
        ...
        local function SelectTab()
            ...
            TweenService:Create(TabContent, fadeInTweenInfo, {BackgroundTransparency = 1}):Play()
            TweenService:Create(TitleFrame, fadeInTweenInfo, {BackgroundTransparency = 1}):Play()
            
            for _, child in pairs(Container:GetChildren()) do
                if child:IsA("Frame") then
                    local originalBgTransparency = 0.7
                    if child.Name == "Section" then
                        originalBgTransparency = 1
                    end
                    
                    child.BackgroundTransparency = 1
                    TweenService:Create(child, fadeInTweenInfo, {BackgroundTransparency = originalBgTransparency}):Play()
                    
                    for _, subChild in pairs(child:GetDescendants()) do
                        if subChild:IsA("GuiObject") and subChild.BackgroundTransparency ~= 1 then
                            local originalTransparency = subChild.BackgroundTransparency
                            subChild.BackgroundTransparency = 1
                            TweenService:Create(subChild, fadeInTweenInfo, {BackgroundTransparency = originalTransparency}):Play()
                        end
                        if subChild:IsA("TextLabel") or subChild:IsA("TextButton") or subChild:IsA("TextBox") then
                            local originalTextTransparency = subChild.TextTransparency
                            subChild.TextTransparency = 1
                            TweenService:Create(subChild, fadeInTweenInfo, {TextTransparency = originalTextTransparency}):Play()
                        end
                        if subChild:IsA("ImageLabel") or subChild:IsA("ImageButton") then
                            local originalImageTransparency = subChild.ImageTransparency
                            subChild.ImageTransparency = 1
                            TweenService:Create(subChild, fadeInTweenInfo, {ImageTransparency = originalImageTransparency}):Play()
                        end
                    end
                end
            end
    ...
    task.spawn(function()
        ...
        elseif tabOrder > 0 then
            ...
            if firstTab then
                ...
                firstTab.BorderColor3 = Color3.fromRGB(39, 39, 42)
